...
4. 사용자가 **Income** 또는 **Expenses** 링크를 클릭하면, 같은 페이지 **하단 영역**에 해당 **펀드(Fund) + 동일 보고 기간 + 클릭한 유형(Income vs Expenses)** 에 맞는 **Detail 목록**을 표시한다.  
   - Detail은 General Ledger 적재 행을 **Credit 위주(Income) / Debit 위주(Expenses)** 로 필터해 보여주는 규칙을 문서화한다(아래 §4 참고).
...
- `fund_activity_detail_line` — 테이블명 동일  
  ...
  - `line_date`, `transaction_number`, `transaction_type`(엑셀 `Type`), `contact`, `memo`, `reference_number`, `note`  
  - `debit`, `credit`, `amount`, `balance` — `DECIMAL(20,6)`  
  - `account_section` — 상위 계정 구간(`Name` 열 중 제목 행) 전파 값(선택)  
...
Summary의 **Income**, **Expenses** 는 QB Fund Activity 집계값이다. Detail은 **원장 라인**이므로 다음을 전제로 한다.

- **Income 링크**: 동일 `run_id`, 동일 `fund_name`, 기간 내 라인 중 **`Credit > 0`** (또는 운영 확정 규칙에 따라 Credit 관련만) 을 나열한다.  
- **Expenses 링크**: 동일 조건에서 **`Debit > 0`** (또는 Debit 관련만) 을 나열한다.

한 라인에 Debit·Credit이 동시에 있는 경우는 **제품 정책**으로 한쪽만 표시할지, 둘 다 표시하고 태그할지 정한다.  
Summary 합계와 Detail 라인 합이 **소수·반올림** 때문에 완전 일치하지 않을 수 있음을 UI에 작은 문구로 안내할 수 있다.
...
- API: `GET …?runId=&fundName=&kind=income|expenses` 형태로 **기 적재된** Detail 목록을 반환(Income/Credit·Expense/Debit 필터는 §4).  
