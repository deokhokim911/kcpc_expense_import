...
**구현 상태 (2026-06-15)**: 본 문서는 **설계·스펙**용. Go 적재 코드(`org_expense_summary`)는 **이전 `handleMinistryBalance` 방식으로 복원**됨. DB에 테이블을 이미 만든 환경은 [`rollback.sql`](20260606120000_add_org_expense_summary/rollback.sql) 로 제거 가능.
...
| Phase | 작업 | UI/기존 테이블 | 상태 |
|-------|------|----------------|------|
| **0** | 현 UI 집계 SQL 확정 (검증용) | 무변경 | 대기 |
| **1** | migration CREATE / **rollback.sql** | 무변경 | DDL만 레포 보관 |
| **2** | `handleMinistryBalance` + summary 1-pass | 무변경 | **코드 복원됨 (미적용)** |
| **3** | 트랜잭션 + dry-run summary | 무변경 | **코드 복원됨** |
| **4** | §11 검증 SQL | 무변경 | 대기 |
| **5** *(2차·선택)* | UI summary 조회 | UI 변경 | 대기 |

**현재 운영**: ministry 적재는 **기존** `org_balance` delete+insert 만 사용. summary 테이블은 **rollback.sql** 로 DB에서 제거 가능.
...
**파일**: [`2026/20260606120000_add_org_expense_summary/rollback.sql`](20260606120000_add_org_expense_summary/rollback.sql)
...
**주의**: summary·run 테이블 **데이터 삭제**. `org_balance` / `org_budget` / UI는 **영향 없음**.
